	...
	const mixSightings=()=>{
        ...
        if(sorted_sightings.length!==0){
		    for(let k=0;k<user_sightings_count[sorted_sightings[0]];k++){
			    ordered_sightings.push("https://api.theaspenproject.cloud/api/static/user/"+actual_username.replace(" ","_").toLowerCase().replace("<","\\<")+"/images/"+sorted_sightings[0].replace(" ","_").toLowerCase()+"/"+k.toString()+".jpg");
		    }
        }
