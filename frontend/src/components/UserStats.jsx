		    ...
		    for(let l=1;l<sorted_sightings.length;l++){
			    ...
			    for(let m=user_sightings_count[sorted_sightings[l]];m>0;m--){
				    if(places_per_interval*m>=ordered_sightings.length){
					    ordered_sightings.push("https://api.theaspenproject.cloud/api/static/user/"+actual_username.replace(" ","_").toLowerCase()+"/images/"+sorted_sightings[l].replace(" ","_").toLowerCase()+"/"+(m-1).toString()+".jpg")
				    }else{
					    ordered_sightings.splice(places_per_interval*m,0,"https://api.theaspenproject.cloud/api/static/user/"+actual_username.replace(" ","_").toLowerCase()+"/images/"+sorted_sightings[l].replace(" ","_").toLowerCase()+"/"+(m-1).toString()+".jpg")
				    }
			    }
		    }
        }
	...
	return (<div className={styles.UserStatsContainer}>
                <div className={styles.SeenRankingContainer}>
			        {(sortedSeenSpecies.length>0)&&<div className={styles.SeenRanking}>
                        ...
                        {(sortedSeenSpecies.length>2)?<Reccomendations seen={user_seen} reccomendation_tags={[sortedSeenSpecies[0],sortedSeenSpecies[1],sortedSeenSpecies[2]]} introduction="Here are some species that you may like, based on what you have seen already" />:<p>See more species and get reccomendations on where you can see more of what you like</p>}
			        </div>}
			        {(sortedSeenSpecies.length>4)&&<div className={styles.SeenRanking}>
                        ...
                        {(sortedSeenSpecies.length>6)?<Reccomendations seen={user_seen} reccomendation_tags={[sortedSeenSpecies[sortedSeenSpecies.length-1],sortedSeenSpecies[sortedSeenSpecies.length-2],sortedSeenSpecies[sortedSeenSpecies.length-3]]} introduction="Here are some species that might push you out of your comfort zone" />:<p>See more species and get reccomendations on what might surprise you</p>}
			        </div>}
                </div>
